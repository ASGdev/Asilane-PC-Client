...
public class Synthesiser {
	...
	/**
	 * Returns the current language code for the Synthesiser. Example: English(Generic) = en, English (US) = en-US,
	...
	/**
	 * Note: set language to auto to enable automatic language detection. Setting to null will also implement Google's
	 ...
	 */
	public InputStream getMP3Data(final String synthText) throws Exception {

		if (synthText.length() > 100) {
			final List<String> fragments = parseString(synthText);// parses String if too long
		...
		final URL url = new URL(GOOGLE_SYNTHESISER_URL + languageCode + "&q=" + encoded); // create url
	 ...
	 */
	public InputStream getMP3Data(final List<String> synthText) throws Exception {
		InputStream complete = getMP3Data(synthText.remove(0));
		for (final String part : synthText) {
	 ...
	 * @param fragments
	 *            List<String> that you want to add stuff too. If you don't have a List<String> already constructed
	 *            "new ArrayList<String>()" works well.
	...
	/**
	 * Finds the last word in your String (before the index of 99) by searching for spaces and ending punctuation. Will
	 * preferably parse on punctuation to alleviate mid-sentence pausing
	 * 
	 * @param input
	...
	/**
	 * Checks if char is an ending character Ending punctuation for all languages according to Wikipedia (Except for
	 * Sanskrit non-unicode)
	 * 
	 * @param The
	 *            char you want check
